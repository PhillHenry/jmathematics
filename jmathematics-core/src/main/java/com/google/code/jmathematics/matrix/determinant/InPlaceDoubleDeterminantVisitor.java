...
public class InPlaceDoubleDeterminantVisitor implements
    ...
    public void checkSquare(Double2DMatrix matrix) {
        if (matrix.getHeight() != matrix.getWidth()) throw NOT_SQUARE_X;
    }

    private double determinant(Double2DMatrix matrix) {
        checkSquare(matrix);
        if (matrix.getHeight() != matrix.getWidth()) throw new IllegalStateException("Not a square matrix");
        int         n               = matrix.getHeight();
        double      accumulator     = 0;
        int[][]     permutations    = intPermutation.permutate(n);
        for (int j = 0 ; j < permutations.length ; j++) {
            double term = term(leviCivita, permutations[j], matrix);
            accumulator += term;
        }
        return accumulator;
    }
    
    private double term(LeviCivita leviCivita, int[] indexes, Double2DMatrix matrix) {
